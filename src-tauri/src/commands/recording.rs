...
pub fn start_recording(
    ...
    let started = state.recorder.start(RecorderConfig {
...
#[tauri::command]
pub fn pause_recording(state: State<'_, AppState>) -> AppResult<RecordingSnapshot> {
    let snapshot = state.recorder.pause()?;
    state.db.with_conn(|conn| {
...
#[tauri::command]
pub fn resume_recording(state: State<'_, AppState>) -> AppResult<RecordingSnapshot> {
    let snapshot = state.recorder.resume()?;
    state.db.with_conn(|conn| {
...
#[tauri::command]
pub fn get_recording_state(state: State<'_, AppState>) -> Option<RecordingSnapshot> {
...
#[tauri::command]
pub fn stop_recording(app: AppHandle, state: State<'_, AppState>) -> AppResult<Meeting> {
    let snapshot = state
        .recorder
        .snapshot()
    ...
    let stop_result = state.recorder.stop();
    ...
    match stop_result {
        Ok(finalized) => {
            ...
            crate::pipeline::spawn(app, meeting_id);
            Ok(meeting)
        }
        Err(e) => {
...
#[tauri::command]
pub fn get_recoverable_meetings(state: State<'_, AppState>) -> AppResult<Vec<RecoverableMeeting>> {
    let meetings = state.db.with_conn(repo::list_interrupted_meetings)?;

    // 録音中の会議は復旧対象ではない（今まさに動いている）。
    let active_id = state.recorder.snapshot().map(|s| s.meeting_id);
...
#[tauri::command]
pub fn check_disk_status(state: State<'_, AppState>) -> DiskStatus {
    let settings = state.settings.get();
